...
const filteredPayments = payments.filter(p => {
  ...
  const matchDue = !searchDue || p.payment_period_start.startsWith(searchDue); // 날짜 비교 개선
  const matchPaid = !searchPaid || (p.paid_payment && p.paid_payment.startsWith(searchPaid)); // 날짜 비교 개선
  const matchStatus = !searchStatus || p.is_payment === searchStatus;
        ...
        <tbody>
          {filteredPayments.map(p => (
            <tr key={p.id}>
              ...
              <td className="px-4 py-2 text-center w-[130px]">{p.payment_period_start}</td>
              <td className="px-4 py-2 text-center w-[130px]">{p.paid_payment || '-'}</td>
              ...
              <td className="px-4 py-2 text-center text-red-500 w-[130px]">
                {p.is_payment !== '완납' ? p.amount.toLocaleString() : '-'}
              </td>
